...
BEGIN
    ...
    IF NOT FOUND THEN 
        RAISE NOTICE 'El registro en tab_transportadoras no existe.';
        RETURN FALSE; 
    END IF;

    UPDATE tab_transportadoras
    SET nom_transportador = p_nom_transportador,
        tipo_transporte = p_tipo_transporte,
        tel_contacto = p_tel_contacto,
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error transaccional en %: %', 'fun_u_transportadora', SQLERRM;
        RETURN FALSE;
END;
$$ LANGUAGE plpgsql;
